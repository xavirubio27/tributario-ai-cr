...
| 8 | `FE/Emisor/Identificacion/Numero` | `document_parties` | `identification_number` | `text` | **NULL** | — | `string` ≤20, **sin `minLength`** | `length between 0 and 20` · presencia por `role` (§21.2) | **Texto**: admite alfanuméricos (rev. 2026). Nullable en la columna; obligatorio para el **emisor** por `CHECK`. `NULL` = `Identificacion` ausente · `''` = presente con `Numero` vacío (B2-R3) |
...
| 12 | `FE/Receptor/Identificacion/Numero` | `document_parties` | `identification_number` | `text` | **NULL** | — | `string` ≤20, **sin `minLength`** | misma columna | Ídem: la ausencia solo es legal en TE/NC/ND (§21.2). `NULL` ≠ `''` (B2-R3) |
...
| 45 | `FE/InformacionReferencia/Numero` | `document_references` | `reported_number` | `text` | NULL | — | `string` ≤50 `[0..1]`, sin `minLength` | `length between 0 and 50` | Opcional → sin FK obligatoria. `NULL` = ausente · `''` = presente vacío (B2-R1) |
...
| 48 | `FE/InformacionReferencia/Razon` | `document_references` | `reason` | `text` | NULL | — | `string` ≤180 `[0..1]`, sin `minLength` | `length between 0 and 180` | `NULL` = ausente · `''` = presente vacío (B2-R1) |
